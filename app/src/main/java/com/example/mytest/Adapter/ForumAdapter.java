...
import com.example.mytest.CommentActivity;
...
public class ForumAdapter extends RecyclerView.Adapter<ForumAdapter.ViewHolder> {
    ...
    public ForumAdapter(Context ct, List<ForumPost> forumPostList, String imageurl1) {
        context = ct;
        mForumPost = forumPostList;
        imageurl = imageurl1;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ForumPost forumpost = mForumPost.get(position);

        holder.show_message.setText(forumpost.getMessage());
        holder.show_vote.setText(forumpost.getVote());

        // if() {
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference("User").child(forumpost.getSender());
        ...
        holder.comment.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                intent.putExtra("catid", forumpost.getCatid());
                context.startActivity(intent);

            }
        });

        DatabaseReference voteRef = FirebaseDatabase.getInstance().getReference("Votelist").child(forumpost.getId()).child(fuser.getUid());
        ...
        if(forumpost.getPoint().equals("true")) {
            DatabaseReference reference2 = FirebaseDatabase.getInstance().getReference("User").child(forumpost.getSender());
            reference2.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    if (user.getOccupation().equals("student")) {
                        ...
                        FirebaseDatabase.getInstance().getReference("User").child(forumpost.getSender()).child("points").setValue(total_point);
                        FirebaseDatabase.getInstance().getReference("Student").child(forumpost.getSender()).child("points").setValue(total_point);
                        FirebaseDatabase.getInstance().getReference("ForumPost").child(forumpost.getId()).child("point").setValue("earned");
                    }
                }
        ...
        holder.up.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){
                String votes = forumpost.getVote();
                
                DatabaseReference reference1 = FirebaseDatabase.getInstance().getReference("Votelist").child(forumpost.getId()).child(fuser.getUid());
                ...
                reference1.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if(a.equals("false")){
                            ...
                            FirebaseDatabase.getInstance().getReference("ForumPost").child(forumpost.getId()).child("vote").setValue(total_votes);
                            reference1.child("downvoted").setValue("false");
                            reference1.child("upvoted").setValue("true");
                            if(total_votes.equals("10") && forumpost.getPoint().equals("false")){
                                FirebaseDatabase.getInstance().getReference("ForumPost").child(forumpost.getId()).child("point").setValue("true");
                            }
                        }
                    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            super(itemView);
            show_message = itemView.findViewById(R.id.post_message);
